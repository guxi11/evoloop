...
import { tomlTables, walk } from '../util.mjs'
...
// Skills the package itself ships. They describe how to drive evoloop, so they
// belong to the tool, not to whatever config repo it happens to render — the
// data repo stays pure user material. Anchoring on import.meta.url is correct
// here and nowhere else: this reads the package's own files.
const BUILTIN_SKILLS = fileURLToPath(new URL('../../skills', import.meta.url))

const builtinFiles = dir =>
  walk(BUILTIN_SKILLS).map(p => [`${dir}/${relative(BUILTIN_SKILLS, p)}`, readFileSync(p)])
...
export const render = (spec, ir) => {
  ...
  return {
    ...
    files: Object.fromEntries([
      ...
      // Builtins first: a repo skill of the same name overrides ours.
      ...(spec.skillsDir ? builtinFiles(spec.skillsDir) : []),
